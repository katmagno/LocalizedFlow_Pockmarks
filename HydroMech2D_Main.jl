...
@views function HydroMech2D()
    ...
    ENV["GKSwstype"]="nul";
    if isdir(string(nperm) * "[INSERT]")==false mkdir(string(nperm) * "[INSERT]") end; loadpath = "./" * string(nperm) * "[INSERT]" * "/"; anim = Animation(loadpath,String[])
    ...
    while t< t_tot
        ...
        path_name = "[INSERT PATH TO SAVE DATA]"
        if mod(it,100)==0
            fnm_qDys = path_name * "[INSERT].csv" # Save qDy values at each time step
            writedlm(fnm_qDys, Array(qDys), ',')
            fnm_Phi = path_name * "[INSERT].csv" # Save Phi values at each time step
            writedlm(fnm_Phi, Array(Phi), ',')
        end
    end
    path_name = "[INSERT]"
    fnm_times = path_name * "[INSERT].csv"
    fnm_gif = path_name * "[INSERT].gif"
